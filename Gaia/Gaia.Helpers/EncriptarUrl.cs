...
{
    public static class EncriptarUrl
    {
        /// <summary>
        ...
        /// <returns></returns>
        public static string EncriptaString(string url)
        {
            try
            {
                byte[] clave = Encoding.UTF8.GetBytes("routeController");
                byte[] vInicio = Encoding.UTF8.GetBytes("accionIr");
                string e = "";
                //e  = RijndaelEncriptar(url, clave, vInicio);
                e = tripleDES_Encrypt(url, true);

                if (!url.Equals(tripleDES_Decrypt(e, true)))
                { return ""; }


                string cadenaDepurada = e.Replace("+", "$").ToString();
                cadenaDepurada = cadenaDepurada.Replace("=", "[").ToString();
                cadenaDepurada = cadenaDepurada.Replace("/", "]").ToString();
                e = "Route/Ir/" + cadenaDepurada;
                string laUrl = e;
                return laUrl;
        ...
        public static string DesencriptaString(string cadenaEncriptada)
        {
            byte[] clave = Encoding.UTF8.GetBytes("routeController");
            byte[] vInicio = Encoding.UTF8.GetBytes("accionIr");

            string cadenaDepurada = cadenaEncriptada.Replace("$", "+").ToString();
            cadenaDepurada = cadenaDepurada.Replace("[", "=").ToString();
            cadenaDepurada = cadenaDepurada.Replace("]", "/").ToString();

            //return RijndaelDesEncriptar(cadenaEncriptada, clave, vInicio);
            return tripleDES_Decrypt(cadenaDepurada, true);
        }
